describe('Prueba del Flujo de Inicio de Sesión', () => {
    ...
    it('debería navegar a la página de inicio de sesión e iniciar sesión correctamente', () => {
      ...
      cy.get('#AddressTitle').type('Dirección a modificar');
      ...
      cy.contains("Dirección a modificar")
      .parents('.profile-details-address-details ')
      .find('.profile-details-addresses-edit-button').click();  

      cy.get('#AddressTitle').clear().type('Dirección modificada');

      cy.wait(1000);
  
      cy.get('.form-buttons button[type="submit"]').click();

      cy.contains('Dirección modificada').should('be.visible');
    });
  });